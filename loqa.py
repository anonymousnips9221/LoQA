...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    evaluation_strategy: str = field(default='steps', metadata={"help": 'When to save checkpoints'})
    save_steps: int = field(default=250, metadata={"help": 'How often to save a model'})
    eval_steps: int = field(default=250, metadata={"help": 'How often to save a model'})
...
class SavePeftModelCallback(transformers.TrainerCallback):
    def save_model(self, args, state, kwargs):
        ...
        save_loqa(peft_model_path,kwargs["model"])
...
def get_accelerate_model(args, checkpoint_dir):
    ...
    for name, param in model.named_parameters():
        if any([x in name for x in ['lora',"ssf"]]):
...
def flan_v2_path_to_dataset(path):
    dataset = load_json_dataset(path)
    dataset = dataset.rename_column("target", "output")
    dataset = dataset.rename_column("source", "input")
    dataset = dataset.remove_columns('task_name')
    dataset = dataset.remove_columns('task_source')
    dataset = dataset.remove_columns('template_type')
    dataset = dataset.remove_columns('template_idx')
    return dataset
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    elif args.dataset == "flan-v2":
        train_dataset = flan_v2_path_to_dataset(
            "path_to_train_data"
        )
        eval_dataset = flan_v2_path_to_dataset(
            "path_to_eval_data"
        )
        dataset = DatasetDict({'train': train_dataset, "eval": train_dataset})
        ...
        tokenizer.add_special_tokens(
            {
                ...
                "unk_token": tokenizer.convert_ids_to_tokens(model.config.pad_token_id if model.config.pad_token_id != -1 else 0),
            }
        )
