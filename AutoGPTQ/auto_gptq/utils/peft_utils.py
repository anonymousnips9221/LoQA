...
class GPTQLoraLinear(torch.nn.Linear, LoraLayer):
    def __init__(
        ...
        **kwargs,
        ...
        self.zp_lora_A = torch.nn.Parameter(torch.zeros(linear_module.in_features // group_size, r))
        self.zp_lora_B = torch.nn.Parameter(torch.nn.init.xavier_uniform_(torch.randn(r, linear_module.out_features)))
        self.zp_lora_scaling = lora_alpha / r 
        self.scale_lora_A = torch.nn.Parameter(torch.zeros(linear_module.in_features // group_size, r))
        self.scale_lora_B = torch.nn.Parameter(torch.nn.init.xavier_uniform_(torch.randn(r, linear_module.out_features)))
        self.scale_lora_scaling = lora_alpha / (r * self.linear_module.maxq)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            result = self.linear_module(
                x,
                zp_lora_A=self.zp_lora_A,
                zp_lora_B=self.zp_lora_B,
                zp_lora_scaling=self.zp_lora_scaling,
                scale_lora_A=self.scale_lora_A,
                scale_lora_B=self.scale_lora_B,
                scale_lora_scaling=self.scale_lora_scaling,
            )
