...
class QuantLinear(nn.Module):
    ...
    def forward(
        ...
        zp_lora_A=None,
        zp_lora_B=None,
        zp_lora_scaling=None,
        scale_lora_A=None,
        scale_lora_B=None,
        scale_lora_scaling=None,
    ):
        ...
        if self.autogptq_cuda_available is True and (
            ...
            if self.bits in [2,4,8]:
                ...
                if zp_lora_A is not None:
                    zeros = self.qzeros + (zp_lora_A @ zp_lora_B * zp_lora_scaling).half()
                else:
                    zeros = self.qzeros
                zeros = zeros.reshape(-1, 1, zeros.shape[-1])

                if scale_lora_A is not None:
                    scales = self.scales+(scale_lora_A @ scale_lora_B * scale_lora_scaling).half()
                else :
                    scales = self.scales
